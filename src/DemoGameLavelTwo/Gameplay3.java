...
public class Gameplay3 extends JPanel implements KeyListener, ActionListener {
	...
	private MapGenerator3 mapob;//generator the outlining of bricks
	// new
	private int totalbrick = 62;//number of total bricks
	private Font f, f2;//declares variable under font class
        ...
        public static int life=Gameplay2.life;//a variable declared to keep track of the score,which is initially Level2's life  
        
        public static  int score = Gameplay2.score;//a variable declared to keep track of the score,which is initially Level2's score
	// second ball
	private boolean isSecondBallAppear = false;
	private int secondBallPosX = 740;//position of the second ball along x-axis
	private int secondBallPosY = 250;//position of the second ball along y-axis
	...
	private int secondBallStartBrick = 45;//total bricks that would be present during the arrival of the second ball
	...
	public void paint(Graphics g) {
		...
		g.drawString("Score: " + score, 985, 30);
		...
		if (play) {
			...
			A: for (int i = 0; i < mapob.map.length; i++) {
				for (int j = 0; j < mapob.map[0].length; j++) {
					if (mapob.map[i][j] > 0) {
						int brickX = j * mapob.brickwidth ;
						int brickY = i * mapob.brickheight+20 ;
						...
						if (ballrect.intersects(brickrect)) {
                                                       playMusic("Music\\\\hitperfect.wav");
                                                         ...
                                                         if(mapob.map[i][j]==3)
                                                                mapob.setBrickValue(2, i, j);
                                                         else if(mapob.map[i][j]==2)
